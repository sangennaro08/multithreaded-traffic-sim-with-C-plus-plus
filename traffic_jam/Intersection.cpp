...
void Incrocio::simula_passaggio(Auto* A, std::vector<std::unique_ptr<Incrocio>>& Incroci)
{
    ...
    for(int i = 0; i < (int)Incroci.size() || (finish_last_interception != last_intercetion && finish_last_interception != 0); i++)
    {
        if(Incroci.at(idx)->entrare.try_acquire())
        {
...

            {
                ...
                std::cout << "L'auto " << A->nome << " è arrivato all'incrocio "
                          << A->incrocio_iniziale << " adesso si deciderà in quale direzione del prossimo incrocio andrà\n\n";
            }
            ...
            if(idx != last_intercetion && i < Incroci.size()) 
    ...
    get_out_intersection(Incroci.at(A->incrocio_iniziale), A);
