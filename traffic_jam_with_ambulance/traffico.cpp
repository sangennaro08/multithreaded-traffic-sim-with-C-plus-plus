...
int main()
{
    ...
    std::vector <Veicolo*> Ambulanze;

    Incrocio::inizializza_Incroci(Incroci);
    Incrocio::inizializza_auto(Auto_in_gioco,Incroci);
    Incrocio::create_ambulances(Ambulanze);
    
    std::cout<<"Sono state create "<<Ambulanze.size()<<" possibili da utilizzare...\n\n";
    std::this_thread::sleep_for(std::chrono::seconds(3));

    Incrocio::lancia_auto(Auto_in_gioco,Incroci,Ambulanze);

    {
        std::unique_lock <std::mutex> lock(finisci_programma);

        continua.wait(lock,[&]{return Auto_completate >= auto_presenti && Ambulance_here == 0;});
    }

    Incrocio::join_threads(Auto_in_gioco, Ambulanze);
