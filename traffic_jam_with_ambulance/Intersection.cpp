...
void Incrocio::lancia_auto(std::vector<Veicolo*>& Auto_in_gioco, std::vector<std::unique_ptr<Incrocio>>& Incroci)
{
...
void Incrocio::join_threads(std::vector<Veicolo*>& Auto_in_gioco)
{
...
void Incrocio::simula_passaggio(Veicolo* A, std::vector<std::unique_ptr<Incrocio>>& Incroci)
{
    ...
    //TODO fare funzione per determinare se l'ambulanza deve entrare nell'incrocio 0
    //e vedere come fare elegantemente il casting
    //TOO fare override 

    //Qua il reset avviene solo una volta quando ambulanza è in gioco!!!
    //non la sto chiamando direttamente ma con polimorifmso è ignorato da ogni auto fino a che è un ambulanza
    A->reset_ambulance();
    //TODO notare che la funzione non aggiunge percentuale nella simulazione non nascono ambulanze
    Amb->add_percentage();
    //Ambulanza::add_percentage(Amb);
    /*if(Ambulanza* a = dynamic_cast<Ambulanza*>(Amb))
    {
        a->percentage += 0.1f;
    }*/
