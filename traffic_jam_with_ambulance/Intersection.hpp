...
class Incrocio
{
    ...
    static void lancia_auto(std::vector<Veicolo*>& Auto_in_gioco, std::vector<std::unique_ptr<Incrocio>>& Incroci);
    static void join_threads(std::vector<Veicolo*>& Auto_in_gioco);

    private:

    static int decide_interception(std::vector<std::unique_ptr<Incrocio>>& Incroci);
    void simula_passaggio(Veicolo* A, std::vector<std::unique_ptr<Incrocio>>& Incroci);
    ...
    void set_directions(std::unique_ptr<Incrocio>& Actual_Incrocio, Veicolo* A);
    void get_out_intersection(std::unique_ptr<Incrocio>& Actual_Incrocio, const Veicolo* A);
};